(function(window){
    ...
    Seed.prototype = {
        ...
        clear: function() {
            ...
            // Original clear approach - matches exactly what was there before
            var point = cirle.point, scale = cirle.scale, radius = 26;
            var w = h = (radius * scale);
            ctx.clearRect(point.x - w, point.y - h, 4 * w, 4 * h);
            
            // Additional minimal clear for line extension area
            // Only clear if line extends beyond original clear area
            var heartPoint = heart.point, heartScale = heart.scale;
            var originalRight = point.x - w + (4 * w);
            var lineEndX = heartPoint.x + (130 * heartScale);
            
            // Only clear line if it extends beyond original clear and scale is still large enough
            if (lineEndX > originalRight && heartScale > 0.3) {
                var lineStartX = originalRight;
                var extensionWidth = Math.min(lineEndX - lineStartX + (3 * heartScale), 140 * heartScale);
                var extensionHeight = 25 * heartScale;
                var extensionTop = heartPoint.y - (8 * heartScale);
                ctx.clearRect(lineStartX, extensionTop, extensionWidth, extensionHeight);
            }
        },
        hover: function(x, y) {
